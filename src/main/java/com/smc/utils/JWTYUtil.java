...
public class JWTYUtil {
	...
	public String createJWT(UserModel userData) {
		try {
			...
			claims.put(Constants.USER_ID, String.valueOf(userData.getUserId()));
			claims.put(Constants.USER_NAME, userData.getUserName());
			claims.put(Constants.USER_MESSAGE, userData.getUserMessage());

			// Let's set the JWT Claims
			JwtBuilder builder = Jwts.builder().setClaims(claims).setId(String.valueOf(userData.getUserId())).setIssuedAt(now).setSubject("Authentication Token").setIssuer("Mexus.CoooM").signWith(signatureAlgorithm, signingKey);
